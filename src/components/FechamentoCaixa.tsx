...
export const FechamentoCaixa = ({ historicoVendas, paymentTotals, onRefresh, onClose }: FechamentoCaixaProps) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem', height: 'calc(100vh - 160px)', overflowY: 'auto', paddingRight: '4px' }}>
      ...
      {/* Banner turno */}
      <div style={{ background: 'linear-gradient(135deg, rgba(220,38,38,0.15) 0%, rgba(220,38,38,0.05) 100%)', border: '1px solid rgba(220,38,38,0.3)', borderRadius: '16px', padding: '1rem 1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <Clock size={20} color="#dc2626" />
          <div>
            <div style={{ fontSize: '0.65rem', opacity: 0.5, fontWeight: 700 }}>TURNO EM ANDAMENTO • O.S. #{osNumber || '---'}</div>
            <div style={{ fontWeight: 900, color: '#dc2626' }}>Início: {new Date(turnoInicio).toLocaleTimeString('pt-BR')} • Duração: {tempoTurno}</div>
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isGestor ? 'repeat(4, 1fr)' : 'repeat(3, 1fr)', gap: '1rem' }}>
        {[
          ...
          { label: 'PIX', icon: Smartphone, val: paymentTotals.pix, color: '220,38,38', qtd: pedidosPorMetodo.pix.length },
      ...
      {isGestor && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem' }}>
          <div style={{ ...cardStyle('220,38,38'), gridColumn: 'span 2' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <div style={{ fontSize: '0.65rem', opacity: 0.5, fontWeight: 700 }}>TOTAL GERAL DO TURNO</div>
                <div style={{ fontSize: '2.2rem', fontWeight: 900, color: '#dc2626' }}>R$ {totalVendas.toFixed(2)}</div>
              </div>
      ...
      <div style={{ background: 'rgba(220,38,38,0.04)', border: '2px solid rgba(220,38,38,0.2)', borderRadius: '20px', padding: '1.5rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '1.2rem' }}>
          <DollarSign size={20} color="#dc2626" />
          <h3 style={{ color: '#dc2626', fontSize: '0.85rem', fontWeight: 900, margin: 0 }}>CONFERÊNCIA DE GAVETA</h3>
        </div>
          ...
          <div>
            ...
            {profile?.role === 'dono' ? (
              <input
                ...
                style={{ width: '100%', background: 'rgba(255,255,255,0.05)', border: '1px solid #dc2626', borderRadius: '10px', padding: '0.7rem 1rem', color: '#dc2626', fontSize: '1.1rem', fontWeight: 700, outline: 'none' }}
              />
            ) : (
              <div style={{ width: '100%', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '10px', padding: '0.7rem 1rem', color: '#dc2626', fontSize: '1.1rem', fontWeight: 700 }}>
        ...
        <div style={{ marginBottom: '1rem' }}>
          ...
          <input
            ...
            style={{ width: '100%', background: dinheiroGaveta ? 'rgba(220,38,38,0.05)' : 'rgba(255,255,255,0.03)', border: `2px solid ${dinheiroGaveta ? '#dc2626' : 'rgba(255,255,255,0.1)'}`, borderRadius: '12px', padding: '1rem', color: '#fff', fontSize: '1.8rem', fontWeight: 900, textAlign: 'center', outline: 'none', transition: '0.2s' }}
          />
        </div>

        {dinheiroGaveta !== '' && isGestor && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
            style={{ padding: '1rem', borderRadius: '12px', textAlign: 'center', background: gavetaConferida ? 'rgba(16,185,129,0.1)' : diferenca > 0 ? 'rgba(220,38,38,0.1)' : 'rgba(244,63,94,0.1)', border: `1px solid ${gavetaConferida ? 'rgba(16,185,129,0.3)' : diferenca > 0 ? 'rgba(220,38,38,0.3)' : 'rgba(244,63,94,0.3)'}` }}>
            {gavetaConferida ? (
              ...
              <div>
                <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px', color: diferenca > 0 ? '#dc2626' : '#f43f5e', fontWeight: 900 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', paddingBottom: '2rem' }}>
        <button onClick={handleGerarRelatorio}
          style={{ background: 'rgba(220,38,38,0.1)', border: '2px solid rgba(220,38,38,0.4)', borderRadius: '14px', color: '#dc2626', padding: '1rem', fontWeight: 900, fontSize: '0.85rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }}
              ...
              <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: '12px', padding: '1rem', marginBottom: '1.5rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', textAlign: 'left' }}>
                <div><div style={{ fontSize: '0.6rem', opacity: 0.4 }}>TOTAL VENDAS</div><div style={{ fontWeight: 900, color: '#dc2626' }}>R$ {totalVendas.toFixed(2)}</div></div>
